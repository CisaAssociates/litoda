...
// Validate input - check if both fields are empty first
if (empty($username) && empty($password)) {
    logLoginAttempt($conn, null, '', 'Failed', 'Empty username and password');
...
if (empty($username)) {
    logLoginAttempt($conn, null, '', 'Failed', 'Empty username');
    $_SESSION['login_error'] = 'Wrong username.';
    $_SESSION['old_username'] = '';
    header('Location: ../../pages/login/login.php');
    exit();
}
...
if (empty($password)) {
    logLoginAttempt($conn, null, $username, 'Failed', 'Empty password');
...
try {
    ...
    if ($result->num_rows === 1) {
        ...
        // User not found - this means username is wrong
        // Since we already validated both fields are filled, and username is wrong,
        // we can't determine if password is also wrong without revealing that username doesn't exist
        logLoginAttempt($conn, null, $username, 'Failed', 'User not found');
        // Changed to generic message for security - don't reveal which field is wrong
        $_SESSION['login_error'] = 'Wrong username and password.';
        $_SESSION['old_username'] = $username;
    }
